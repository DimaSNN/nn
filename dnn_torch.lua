...
cmd:text()
cmd:text('Options:')
cmd:option('-type',            'false',      'type: float | cuda')
cmd:option('-seed',            1,           'fixed input seed for repeatable experiments')
cmd:option('-threads',         1,           'number of threads')
cmd:option('-gpuid',           1,           'gpu id')
cmd:option('-train_criterion', 'NLL',       'train_criterion: MSE | NLL - negative log likelihood')
...
local feval = function(x)
  ...
  local outputs = mlp:forward(input)
...
--  print(outputs)
  ...
  for i=1,batch_size do
    ...
    if opt.train_criterion == 'MSE' then
      ...
      -- estimate df/dW
      df_do[i] = criterion:backward(outputs[i], targets_matrix[i])
    elseif opt.train_criterion == 'NLL' then
      -- get error
      err = criterion:forward(outputs[i], output[i])
      -- estimate df/dW
      df_do[i] = criterion:backward(outputs[i], output[i])
    end    
    f = f + err
  end
  print (f)
...
function f(x)
  ...
  local df_do = torch.Tensor(Y:size(1), Y:size(2)):float()
	for i=1,batch_size do
		local err = 0;
		-- get error
		err = criterion:forward(Y[i], output[i])
		-- estimate df/dW
		df_do[i] = criterion:backward(Y[i], output[i])
		f = f + err
	end
	print (f)
	-- return f and df/dX
